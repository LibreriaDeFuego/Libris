-- Libris — migración 049: publicar solo texto desde el perfil, y citas
-- destacadas que no dependen de ningún club.
--
-- Dos pedidos, sobre la barra de "compartir" nueva del perfil (arriba
-- del feed de Actividad): tocar la tarjeta debe dejar escribir texto
-- directo, sin abrir el selector de fotos; el ícono, aparte, ofrece
-- agregar una foto O una cita destacada de un libro.
--
-- ============================================================
-- 1. posts: la foto deja de ser obligatoria
-- ============================================================
-- Hasta acá toda publicación del perfil (posts) exigía una foto
-- (image_url not null) — ahora puede ser solo texto, solo foto, o las
-- dos cosas; lo único que no vale es las dos vacías (constraint nueva,
-- del lado de la base, además de la validación ya existente en
-- createPost).
...
-- ============================================================
-- 2. profile_quotes: citas destacadas del perfil, sin club_book_id
-- ============================================================
-- Las citas que ya existían (comments.kind = 'quote') están atadas a un
-- club_book_id — se citan desde ADENTRO de la conversación de un libro
-- de un club, y esa columna es "not null". La barra del perfil no tiene
-- ese contexto (no se está mirando ningún club_book puntual), así que
-- es una tabla aparte, no una fila más de "comments": se guarda directo
-- el título/autor/portada del libro elegido (sin relación con "books"
-- ni "personal_books" — son dos tablas de origen distinto, ver
-- profile_quotable_books más abajo — así que se copian los datos en vez
-- de guardar una referencia).
--
-- Alcance elegido a propósito, más chico que una cita de club: se puede
-- publicar y borrar, y le pueden poner "me gusta" — no tiene hilo de
-- respuestas (profile_quotes no es un tipo de "comments", no hay dónde
-- colgar una respuesta) ni se puede repostear todavía. Se puede sumar
-- después si hace falta.
create table if not exists public.profile_quotes (
...
alter table public.profile_quotes enable row level security;

drop policy if exists "cualquiera ve las citas del perfil" on public.profile_quotes;
create policy "cualquiera ve las citas del perfil"
  on public.profile_quotes for select to authenticated
  using (true);

drop policy if exists "cada quien agrega las suyas" on public.profile_quotes;
create policy "cada quien agrega las suyas"
  on public.profile_quotes for insert to authenticated
  with check (profile_id = auth.uid());

drop policy if exists "cada quien borra las suyas" on public.profile_quotes;
create policy "cada quien borra las suyas"
  on public.profile_quotes for delete to authenticated
  using (profile_id = auth.uid());

-- "Me gusta" de una cita del perfil — mismo criterio que post_likes
-- (migración 029), tabla propia en vez de reusar comment_likes (esa
-- referencia comments.id, esto es otra tabla).
create table if not exists public.profile_quote_likes (
  id uuid primary key default gen_random_uuid(),
  quote_id uuid not null references public.profile_quotes(id) on delete cascade,
  profile_id uuid not null references public.profiles(id) on delete cascade,
  created_at timestamptz not null default now(),
  unique (quote_id, profile_id)
);

alter table public.profile_quote_likes enable row level security;

drop policy if exists "cualquiera ve los me gusta de citas" on public.profile_quote_likes;
create policy "cualquiera ve los me gusta de citas"
  on public.profile_quote_likes for select to authenticated
  using (true);

drop policy if exists "cada quien pone su me gusta" on public.profile_quote_likes;
create policy "cada quien pone su me gusta"
  on public.profile_quote_likes for insert to authenticated
  with check (profile_id = auth.uid());

drop policy if exists "cada quien saca su me gusta" on public.profile_quote_likes;
create policy "cada quien saca su me gusta"
  on public.profile_quote_likes for delete to authenticated
  using (profile_id = auth.uid());

-- ============================================================
-- 3. profile_quotable_books: qué libros se pueden citar
-- ============================================================
-- Los de tus clubes (cualquiera del que seas miembro — no hace falta
-- haberlo terminado, se puede citar algo que estás leyendo ahora mismo)
-- más los que agregaste a mano en Mi Biblioteca. "distinct"/"union"
-- (sin ALL) para no repetir el mismo libro dos veces si aparece por más
-- de un lado (dos clubes leyendo el mismo libro, por ejemplo).
create or replace function public.profile_quotable_books(target_profile_id uuid)
returns table (title text, author text, cover_url text)
language sql security definer stable set search_path = public
as $$
  select distinct b.title, b.author, b.cover_url
    from club_books cb
    join club_members m on m.club_id = cb.club_id
    join books b on b.id = cb.book_id
   where m.profile_id = target_profile_id
  union
  select distinct title, author, cover_url
    from personal_books
   where profile_id = target_profile_id
  order by title;
$$;

revoke all on function public.profile_quotable_books(uuid) from public;
grant execute on function public.profile_quotable_books(uuid) to authenticated;

-- ============================================================
-- 4. profile_activity / recent_activity: suman las citas del perfil
-- ============================================================
-- Mismo "returns table(...)" que ya tenían (migración 044) — no cambia
-- ninguna columna, así que alcanza con "create or replace" (no hace
-- falta borrar la función primero). Se agrega una rama nueva por unión,
-- "citas propias del perfil": kind = 'quote' igual que una cita de
-- club, pero con club_id NULL — así el frontend (ActivityCard.jsx)
-- distingue una de otra y sabe que acá no hay hilo de respuestas ni
-- reposteo, solo "me gusta" (profile_quote_likes). Al no venir de
-- "comments", nunca puede aparecer reposteada (reposts.comment_id
-- apunta a comments, no a profile_quotes) — no hace falta una rama
-- aparte para eso.
  ...
  select id, kind, title, body, voice_transcript, created_at,
    ...
    from (
      ...
      select c.id, c.kind, c.title, c.body, c.voice_transcript, c.created_at,
             ...
             c.quote_style, c.quote_image_url,
      ...
      union all
      -- citas propias del perfil (nuevo, migración 049) — sin hilo de
      -- respuestas ni repost, solo "me gusta".
...
create or replace function public.recent_activity(limit_count int default 30)
returns table (
...
as $$
  select id, kind, title, body, created_at, profile_id, display_name, avatar_url,
    ...
    from (
      ...
      union all
      -- citas del perfil, de cualquier persona (nuevo, migración 049)
