-- Libris — migración 044: la foto de perfil también se ve en las
-- respuestas del feed de Inicio/Perfil (no solo en el comentario/reseña
-- raíz).
--
-- Mismo bug que ya se arregló en la pantalla de Comentarios del club
-- (EngagementBlock ya sabe mostrar la foto real si "reply.profiles.
-- avatar_url" viene con dato — src/components/EngagementBlock.jsx) pero
-- acá el dato ni siquiera llegaba: profile_activity/recent_activity arman
-- cada respuesta a mano con jsonb_build_object, y su "profiles" anidado
-- solo llevaba "display_name", nunca "avatar_url" — así que en el feed de
-- Inicio/Perfil las respuestas siempre mostraban iniciales, tuviera o no
-- foto quien respondió.
--
-- El arreglo es puntual: se agrega 'avatar_url' al "profiles" de cada
-- respuesta (tanto las de un comentario, ligadas a "comments", como las
-- de una foto, ligadas a "post_comments") en las 4 ramas de cada función
-- que arman replies. No cambia ninguna otra cosa: el resto de las
-- funciones es idéntico a la migración 042 (la última que las redefinió),
-- así que no hace falta volver a revisar el "returns table(...)" ni el
-- conteo de columnas del select externo — "replies" ya era jsonb, esto
-- solo enriquece lo que hay ADENTRO de ese jsonb.
