-- Libris — migración 033: comentarios en las fotos.
--
-- Las fotos nunca tuvieron dónde comentar — solo el texto que la propia
-- persona escribe al publicarlas. "post_comments" es una tabla nueva, no
-- una fila más en "comments": esa tabla exige club_book_id (not null),
-- porque hasta ahora todo lo que vivía ahí colgaba de un libro de un club
-- — una foto no cuelga de ningún club, así que no encaja ahí.
--
-- Más simple que el hilo de reseñas/citas a propósito: comentarios de
-- fotos son una lista plana (sin responder a un comentario puntual) y sin
-- su propio "me gusta" — si hace falta eso más adelante, se suma después.
...
-- Mismo criterio que "posts" (schema.sql, "cualquiera ve las fotos
-- publicadas"): las fotos son públicas para cualquier autenticado, sus
-- comentarios también.
create policy "cualquiera ve los comentarios de las fotos"
...
-- ============================================================
-- profile_activity y recent_activity: la columna "replies" que agregó la
-- migración 032 (pensada para el hilo de reseñas/citas) ahora también
-- lleva los comentarios de cada foto — incluidos en el mismo lugar para
-- que ActivityCard no tenga que distinguir de dónde vienen.
-- ============================================================
