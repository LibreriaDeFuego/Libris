-- Libris — migración 032: "Responder" también en Inicio y Perfil, para
-- reseñas y citas (los dos tipos que se ven ahí con su propio contenido).
--
-- Hasta ahora recent_activity/profile_activity devolvían una fila por
-- actividad, sin sus respuestas — Responder solo vivía en Comentarios del
-- club, donde ya se trae el arreglo completo de comentarios del libro para
-- armar el hilo a mano. Acá se agrega una columna "replies" (jsonb) con el
-- mismo hilo, calculado con un jsonb_agg — así ActivityCard no necesita
-- una consulta aparte.
