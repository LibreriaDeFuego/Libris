-- Libris — migración 057: "Ver el resto de los comentarios en el club"
-- podía llevar a un lugar equivocado, sin avisar — si una cita, reseña o
-- comentario compartido en el feed (Inicio/Perfil) era de un libro que el
-- club YA DEJÓ (migración 056, "Empezar un libro nuevo"), ese link
-- llevaba igual a Comentarios del club, pero esa pantalla siempre muestra
-- el libro ACTIVO — nunca el que tenía la publicación original. Terminabas
-- en el capítulo equivocado (o en el primero del libro nuevo, por
-- default), sin ninguna pista de qué había pasado.
--
-- recent_activity/profile_activity (migración 052, la versión más
-- reciente de las dos) suman una columna más, `is_current_book`: si el
-- club_book de esa fila sigue siendo el activo de su club. `ActivityCard`
-- usa esto para decidir si mostrar el link de siempre o, en su lugar, un
-- aviso sin link ("Esto es de un libro anterior del club..."). Los
-- comentarios/reseñas/citas de un CLUB siempre tienen de dónde sacar este
-- dato (club_books.is_active); las publicaciones del perfil (fotos, citas
-- de perfil) no tienen concepto de "libro del club" — quedan en `true`
-- (no importa: esa parte de la pantalla ni se muestra para esos casos,
-- `canOpenClub`/`club_id` ya los excluye).
--
-- Postgres no deja agregarle una columna a una función con
-- `create or replace` — hay que borrarla primero (mismo motivo por el que
-- la propia migración 052 hizo lo mismo con recent_activity).
...
as $$
  select id, kind, title, body, voice_transcript, created_at,
         ...
         reposted_by_id, reposted_by_name, reposted_by_avatar, is_current_book
    from (
      ...
      select c.id, c.kind, c.title, c.body, c.voice_transcript, c.created_at,
             ...
             cb.is_active as is_current_book
        from comments c
