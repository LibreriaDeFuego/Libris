-- Libris — migración 042: carrusel de varias fotos (no solo una) al
-- comentar un capítulo, con su corrección de visibilidad.
--
-- Dos cosas en esta migración:
--
-- 1) Un comentario de texto pasa de "una foto" (migración 041,
--    comments.image_url) a "hasta 6 fotos", en una tabla aparte
--    (comment_photos) — necesaria para guardar el ORDEN del carrusel, algo
--    que una sola columna de texto no puede. Escrito para funcionar tanto
--    si la 041 ya se corrió (migra esa foto suelta a la tabla nueva y
--    borra la columna vieja) como si no (el bloque de abajo no hace nada).
--
-- 2) De paso, se corrige la política de Storage que agregó la 041 para
--    LEER esas fotos: exigía ser miembro del club (is_club_member), pero
--    recent_activity ya deja ver un comentario compartido de un club
--    PÚBLICO sin ser miembro — con esa política tal cual, la foto de ese
--    comentario nunca se hubiera podido firmar para alguien así, aunque
--    el comentario en sí sí se veía en Inicio. Se reemplaza por la misma
--    regla que ya usan recent_activity/profile_activity para esa rama:
--    club sin invitación, o sos miembro.

-- ============================================================
-- 1. Tabla "comment_photos" (reemplaza a comments.image_url)
-- ============================================================
...
drop policy if exists "miembros ven las fotos de los comentarios de su club" on public.comment_photos;
create policy "miembros ven las fotos de los comentarios de su club"
  on public.comment_photos for select to authenticated
  using (exists (
...
drop policy if exists "cada quien agrega fotos a su propio comentario" on public.comment_photos;
create policy "cada quien agrega fotos a su propio comentario"
  on public.comment_photos for insert to authenticated
  with check (exists (
    select 1 from public.comments c where c.id = comment_id and c.profile_id = auth.uid()
  ));

drop policy if exists "cada quien borra las fotos de su propio comentario" on public.comment_photos;
create policy "cada quien borra las fotos de su propio comentario"
  on public.comment_photos for delete to authenticated
  using (exists (
    select 1 from public.comments c where c.id = comment_id and c.profile_id = auth.uid()
  ));

-- Si la 041 ya corrió, "comments.image_url" tiene la foto suelta de cada
-- comentario que ya la tenía puesta — se migra a comment_photos (posición
-- 0) y se borra la columna. Si la 041 nunca corrió, la columna no existe
-- y este bloque entero no hace nada (chequea antes de tocarla).
do $$
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'comments' and column_name = 'image_url'
  ) then
    insert into public.comment_photos (comment_id, path, position)
    select id, image_url, 0 from public.comments where image_url is not null;

    alter table public.comments drop column image_url;
  end if;
end $$;

-- ============================================================
-- 2. Storage: misma corrección de visibilidad, para "comment-photos"
-- ============================================================
drop policy if exists "miembros ven las fotos de comentarios de su club" on storage.objects;
...
-- Subir sigue exigiendo ser miembro de verdad (política de la 041, sin
-- cambios) — la visibilidad más amplia de arriba es solo para LEER lo que
-- ya se compartió al feed general.

-- ============================================================
-- 3. profile_activity y recent_activity: "image_url" (una foto) pasa a
--    "image_urls" (jsonb, la lista ordenada de comment_photos) — null
--    salvo en un comentario de texto que tenga alguna.
-- ============================================================
