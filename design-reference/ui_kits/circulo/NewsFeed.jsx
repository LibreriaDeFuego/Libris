function NewsFeed() {
  const { FilterPills, Icon, Avatar } = window.LibrisDesignSystem_f40bc3;
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.map((e,i)=>(
          <div key={i} style={{ display:'flex', gap:12, alignItems:'flex-start', background:'var(--surface-card)', borderRadius:'var(--radius-md)', padding:14, boxShadow:'var(--shadow-sm)' }}>
            <div style={{ width:34, height:34, borderRadius:'50%', background:'var(--surface-sunken)', display:'flex', alignItems:'center', justifyContent:'center', color:e.color, flexShrink:0 }}><Icon name={e.icon} size={16} color={e.color}/></div>
            <div>
        ...
        {filtered.length===0 && <div style={{ color:'var(--text-tertiary)', fontSize:'var(--fs-sm)', padding:'20px 0', textAlign:'center' }}>Sin novedades por ahora.</div>}
      </div>
    </div>
  );
}
