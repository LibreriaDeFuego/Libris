...
**Solo texto, y citas del perfil (migración 049).** Tocar la tarjeta entera abría directo el selector de fotos del celular — no dejaba escribir sin elegir antes una foto. Ahora tocar la tarjeta (avatar + texto fantasma) abre el compositor de texto directo; el ícono, aparte, abre un menú con dos opciones: "Agregar foto" (el flujo de siempre: selector → recorte 3:4 → vuelve al mismo compositor, ahora con la foto adjunta, con una × para sacarla sin perder lo escrito) o "Agregar cita" (un flujo nuevo, aparte).
