...
**El encabezado del perfil pasa a ser una estantería con el avatar encima (migración 045).** Antes: avatar chico a la izquierda, los tres números a la derecha en fila. Ahora: arriba de todo, una fila de portadas — los libros que la persona ya TERMINÓ — una al lado de la otra en orden, con esquinas redondeadas y espacio entre sí; si no entran todas, la fila entera se desliza al costado (como el carrusel de fotos de un comentario). El avatar queda centrado, superpuesto sobre el borde inferior de esa fila (`ProfileHero`, `PerfilScreen.jsx`) — sin libros leídos todavía, se muestra solo, sin la fila. Nombre, usuario, bio y los tres números quedan centrados debajo; el botón de seguir (o el disparador de "Foto", en tu propio perfil) pasa a su propia fila centrada, ya no al lado del nombre. Se armó primero como mockup (artifact aparte, con los tokens reales de la app) y se fue ajustando en varias vueltas antes de llevarlo al código: al principio los libros del fondo iban desenfocados y girados como un collage — se cambió a una fila derecha, nítida, con más separación entre portadas y esquinas redondeadas, siguiendo indicaciones puntuales sobre una referencia visual.
  - **"Libros que terminó" no es lo mismo que `stats.book_count`** (los tres números de siempre) — ese cuenta cualquier libro con progreso registrado (`reading_progress`), esté terminado o no. La señal correcta de "lo terminé" ya existía: una reseña final (`comments.kind = 'review'`, solo se puede dejar después de marcar el último capítulo y tocar el nodo de FIN en Tu camino). Nueva función `profile_books_read` (`security definer`, misma regla de visibilidad que `profile_activity`/`profile_stats`) trae, para cada libro distinto reseñado por esa persona, la portada más reciente con la que se reseñó — si leyó el mismo libro en dos clubes, cuenta una vez.
  - **Una portada sin subir, en esta fila, no puede quedar en blanco** — a diferencia de otros lugares de la app (`PreferenciasScreen`, la tarjeta de club en `MisClubesScreen`) donde el título ya se muestra aparte como texto y el color sólido de respaldo alcanza, acá la portada es lo único que hay: sin el título encima, un libro sin portada sería un rectángulo de color sin ninguna pista de cuál es. `BookCover` (`PerfilScreen.jsx`) le suma el título, con un degradé oscuro abajo para que se lea, solo cuando falta la portada real.

**"Mi biblioteca" (migración 046)** — la estantería del encabezado y el número "Libros" pasan a ser la puerta de entrada a una pantalla propia (`/perfil/biblioteca`, o `/perfil/[profileId]/biblioteca` para la de otra persona), no un adorno: se llega tocando cualquiera de los dos (`ProfileHero` y el bloque de estadísticas quedan envueltos en un `<Link>` a `libraryHref`). Ahí se ve la lista completa, en grilla de tres columnas, con la portada, el título y el autor de cada libro.

  - Hasta acá, `profile_books_read` solo traía libros con una reseña final DENTRO de un club — no había forma de anotar algo leído por fuera de Libris. Se pensó primero como una posible sección nueva en la barra inferior, pero se descartó a propósito: el README ya documenta que "Descubrir" se sacó de esa barra por ser una pestaña de más, así que agregar una quinta para esto iba en contra de ese mismo criterio. Quedó, en cambio, como una pantalla a la que se entra desde el Perfil.
  - Tabla nueva `personal_books` (`profile_id`, `title`, `author`, `cover_url`) para lo agregado a mano, y bucket `personal-book-covers` — misma visibilidad que `posts` (migración 016): contenido personal, no atado a ningún club, así que se ve siempre que se puede ver el perfil, sin el chequeo de club privado que sí les cabe a los libros que vienen de una reseña.
  - `profile_books_read` se redefinió para traer, además, los agregados a mano — un `union all` entre la reseña final de club (como antes) y `personal_books`, con un campo nuevo `source` (`'club'` o `'personal'`) para poder distinguirlos: en Mi Biblioteca hay chips para filtrar por "Todos" / "De mis clubes" / "Agregados por mí", y el botón de borrar (una × sobre la portada) solo aparece en los agregados a mano — una reseña de club se administra desde el club, no desde acá.
  - `stats.book_count` (el número "Libros" de las tres estadísticas) pasa a salir siempre de `booksRead.length` en vez del valor crudo de `profile_stats` — así el número que se toca siempre coincide con la cantidad que se ve al entrar.
  - Agregar un libro reusa el mismo recorte de portada de marco ajustable que ya tienen los libros de club (`CoverCropModal`) — tampoco acá hay una única proporción correcta — y el mismo patrón de `PostComposer`: el recorte se resuelve del lado del navegador y se guarda en estado local; recién al tocar "Agregar" se suben la portada y se inserta la fila juntas, en una sola acción de servidor (`addPersonalBook`).

**Fechas de lectura y Recuento del año (migración 047)**, al estilo del "Year in Books" de Goodreads: cada libro de Mi Biblioteca puede tener cuándo se empezó y cuándo se terminó, y desde ahí se entra a una pantalla nueva con el resumen de cuántos se terminaron en cada año.

  - Dos orígenes, dos formas de conseguir la fecha. Un libro de CLUB la saca sola de `reading_progress`: se le agregó `started_at` (se fija una sola vez, con el default de la columna — `updateProgress`, en `clubs.js`, hace upsert con un payload que nunca incluye ese campo, así que un conflicto nunca lo pisa) y se empezó a leer `finished_at` (ya existía desde la migración 023, para la opción "Terminado" de Actualizar progreso, pero hasta ahora nadie la leía). Un libro AGREGADO A MANO no tiene de dónde sacarlas: se piden directo en el formulario, las dos opcionales.
  - **Dato conocido, no error**: a un libro de club que ya estaba registrado ANTES de esta migración, "empezaste" le va a quedar en la fecha en que se corrió la migración, no en la real — nunca se guardó esa información, así que no hay forma de reconstruirla. De acá en adelante queda bien desde el primer avance que se registre.
  - En el formulario de agregar (y en uno nuevo para editar, `EditPersonalBookForm` — se abre tocando la portada de un agregado a mano, propio) dejar "Terminaste" en blanco significa "todavía lo estoy leyendo": ese libro sigue apareciendo en Mi Biblioteca (al principio de la lista, antes que los ya terminados), pero no cuenta para el Recuento de ningún año hasta que se le ponga una fecha de fin.
  - `profile_books_read` (`reviewed_at` pasa a llamarse `finished_at`, más preciso) devuelve ahora `started_at`/`finished_at` para los dos orígenes — para un libro de club, `finished_at` sale de `reading_progress.finished_at`, con la fecha de la reseña como respaldo si por lo que sea faltara.
  - **El año que cuenta es el de "terminaste", no el de "empezaste"** — mismo criterio que usa Goodreads: un libro que se empezó en diciembre de un año y se terminó en enero del siguiente cuenta para el segundo.
  - `RecuentoScreen` (`/perfil/biblioteca/recuento`, o `/perfil/[profileId]/biblioteca/recuento`) no le pide nada nuevo al servidor — reusa la misma lista que ya trajo Mi Biblioteca (`getLibraryData`) y arma los años a mano, del lado del cliente, agrupando por el año de `finished_at`. Chips arriba para elegir el año (el actual siempre aparece, aunque tenga cero libros), grilla de portadas abajo con la fecha de cada una.

**Migración 048 — dos arreglos, apenas se probó lo de arriba.** Editar la fecha de un agregado a mano parecía funcionar (sin ningún error) pero no quedaba guardado; y no había ninguna forma de corregir la de un libro de club.
  - **El bug de verdad**: `personal_books` (migración 046) tenía policy de select/insert/delete, pero nunca de UPDATE. Con RLS activado y ninguna policy que lo permita, Postgres no tira error — el UPDATE simplemente no toca ninguna fila (PostgREST no distingue eso de "no encontró la fila"), así que el formulario mostraba éxito y cerraba, pero nada cambiaba. Se agregó la policy que faltaba.
  - **Editar un libro de club** (pensado sobre todo para corregir el "empezaste = hoy" que le queda a uno viejo, según ya advertía la migración 047): `reading_progress` sí tenía su policy de update — lo que faltaba era el dato para apuntarlo. `profile_books_read` ahora también devuelve `club_book_id` (null en un agregado a mano), y tocar la portada de un libro de club, en la propia biblioteca, abre `EditClubBookDatesForm` — mismo modal que el de un agregado a mano pero sin título/autor (esos son del libro del club, no se editan desde acá), llama a la acción nueva `updateClubBookDates`.

### Fotos de lo que estás leyendo (migración 016)

Desde el propio perfil se puede compartir una foto de lo que se está leyendo — de la galería o sacada en el momento con la cámara. Antes de subirla pasa por el mismo recorte estilo Instagram que la foto de perfil (`PhotoCropModal`, `src/components/PhotoCropModal.jsx`), acá en proporción vertical 3:4 en vez de cuadrada, con un texto corto opcional.

- **El disparador se movió, y cambió de forma.** Antes era un círculo con + solo, sin texto, centrado debajo de las tres estadísticas (Libros/Seguidores/Siguiendo) — desconectado del feed de Actividad, que es donde termina apareciendo la foto. Ahora es una barra de "compartir" arriba de ese mismo feed (tu avatar + "¿Qué estás leyendo?" + un ícono), mismo patrón de compose-bar que ya se conoce de cualquier red social. Se armaron 7 alternativas de POSICIÓN como mockup (artifact aparte, con los tokens reales de la app) — ícono en el header, botón flotante, pill con etiqueta, adentro del menú de tres puntos, esta barra, una insignia sobre el avatar, un cuarto tile en las estadísticas — antes de elegir esta: es la más reconocible sin necesitar explicación, y queda junto a su resultado en vez de mezclada con el bloque de identidad. `PostComposer` pasa a recibir `profile` (antes no necesitaba ningún dato del perfil) para mostrar el avatar de la barra.
  - El primer relleno (fondo gris `surface-sunken`, sin borde) no convenció — leía como un campo deshabilitado, no como algo para tocar. Se probaron 6 alternativas de COLOR más sobre esa misma barra (tarjeta blanca con borde, tinte coral clarito, solo contorno, relleno coral sólido, línea simple sin pastilla, tarjeta flotante con sombra) y quedó la tarjeta blanca: mismo tratamiento que ya usan las tarjetas y el menú del perfil (`surface-card` + borde `border-default` + `shadow-sm`) — se distingue del fondo de la página sin necesitar un color de relleno aparte.

**Solo texto, y citas del perfil (migración 049).** Tocar la tarjeta entera abría directo el selector de fotos del celular — no dejaba escribir sin elegir antes una foto. Ahora tocar la tarjeta (avatar + texto fantasma) abre la ventana para escribir, con una fila de íconos adentro (`AttachmentToolbar`) para adjuntar una foto, un GIF o pasar a una cita destacada — se pensó primero como un menú aparte, sobre la barra, pero se movió adentro de la ventana: todo lo que se puede agregar a la publicación se ve y se elige desde el mismo lugar donde se escribe, en vez de tener que abrir el ícono ANTES de empezar a escribir.
  - "Foto" y "GIF" son dos íconos separados a propósito, aunque las dos rutas terminan en el mismo `handlePick` (que ya distinguía por el tipo de archivo) — dos `<input type="file">`, cada uno con su propio `accept`, así el selector nativo del celular ya filtra de entrada lo que tiene sentido elegir en cada botón. Sin ícono de "GIF" en Lucide — se usa la sigla como botón, mismo criterio que usa X en su propio compositor (tampoco es un pictograma ahí).
  - Una foto (no GIF) sigue pasando por el recorte vertical (3:4) antes de volver a la ventana, ahora con la vista previa y una × para sacarla sin perder el texto ya escrito. Un GIF (migración 039) sigue sin pasar por el recorte, por la misma razón de siempre: `PhotoCropModal` dibuja en un `<canvas>`, que solo captura un frame.
  - Elegir "Cita" en cualquier momento abandona el texto que se estaba escribiendo — son dos publicaciones distintas (`posts` vs. `profile_quotes`), no se pueden mezclar en una sola.

  - **Publicar solo texto es nuevo** — antes toda fila de `posts` exigía una foto (`image_url not null`); ahora puede ser solo texto, solo foto, o las dos cosas (`posts_content_check`, constraint nueva: lo único que no vale es las dos vacías). `createPost`/`updatePost` (posts.js) y `ActivityCard` (el bloque de imagen no se dibuja si no hay ninguna) se actualizaron para ese caso.
  - **Citas destacadas fuera de un club, nuevo (`profile_quotes`).** Las citas que ya existían (`comments.kind = 'quote'`) cuelgan siempre de un `club_book_id` — se citan desde ADENTRO de la conversación de un libro puntual de un club, esa columna es `not null`. La barra del perfil no tiene ese contexto, así que es una tabla aparte: se elige un libro (de una lista — los de tus clubes, terminados o no, más los agregados a mano en Mi Biblioteca, `profile_quotable_books`) y se copian sus datos (título/autor/portada) directo en la fila de la cita, sin relación con `books` ni `personal_books` — son dos orígenes distintos, no hay una única tabla a la que apuntar.
  - **"Otro libro" (migración 051)** — no todo lo que se quiere citar está en esa lista (un libro que no leíste con tu cuenta de Libris, o del que solo tenés una copia física). `QuoteComposer` suma una segunda forma de elegir: título + autor a mano, con una portada opcional recorteada con `CoverCropModal` (mismo componente que ya usa Mi Biblioteca — tampoco acá hay una única proporción). La portada, si hay, se sube al mismo bucket `personal-book-covers` (mismo límite de 5 MB) recién al publicar, igual que el resto de los blobs que se sostienen en estado local hasta el envío final — pero no queda ninguna fila en `personal_books`: es solo para esta cita puntual, no se agrega a Mi Biblioteca ni vuelve a aparecer en la lista de "tus libros" la próxima vez.
  - **Alcance elegido a propósito, más chico que una cita de club**: se puede publicar, borrar y dar "me gusta" (`profile_quote_likes`, mismo criterio que `post_likes`) — todavía no tiene edición, hilo de respuestas ni se puede repostear (una cita de perfil nunca puede aparecer reposteada: `reposts.comment_id` apunta a `comments`, no a `profile_quotes`). `profile_activity`/`recent_activity` la suman como una rama más (`kind = 'quote'`, `club_id` siempre `null`) — ese `club_id` nulo es lo que usa `ActivityCard` para distinguir una cita de perfil de una de club y mostrar la versión simplificada (sin `EngagementBlock`, solo el corazón).
  - ~~Sin tarjeta armada para compartir todavía — se muestra con el mismo tratamiento genérico que ya tiene cualquier cita sin imagen guardada.~~ Ver migración 050 más abajo: ese tratamiento genérico ya no es la única opción.

**Estética propia para las citas (migración 050), adentro de la app.** El tratamiento genérico de una cita sin imagen guardada (portada de fondo + texto en cursiva, igual que una foto) pasa a ser uno más entre varios — ahora se elige un estilo propio al publicar, tanto para una cita de club como para una del perfil. No confundir con `quote_style`/`quoteCard.js` (la tarjeta que se arma como imagen para compartir en Instagram, un sistema aparte con sus propios 3 estilos) — esto es cómo se ve la cita DENTRO del feed de Libris.

  - Se armaron 8 mockups como artifact (con los tokens reales de la app) y se eligieron 4 — "Comilla" (comilla grande de la tipografía de títulos + portada chica), "Franja" (franja de color al borde), "Centrado" (todo centrado, con una regla dorada) y "Papel" (fondo cálido con una cinta decorativa) — más un color de fondo, elegible sin importar el estilo: 5 opciones (blanco, crema, coral, dorado, noche), todas sacadas de tokens que ya existían, nada de color nuevo inventado para esto.
  - La primera versión de "Papel" tenía una portada pegada con inclinación e imitando una polaroid — no quedaba prolija, se probaron 3 alternativas más y se eligió la más simple: la portada derecha, del mismo tamaño y alineación que ya usan "Franja"/"Centrado", en vez de simular estar "pegada".
  - **Bug real, ya arreglado**: "Papel" ponía la cita adentro de un realce propio en dorado FIJO (intentando algo tipo "resaltador") — al elegir cualquier otro color de fondo, quedaban dos cuadrados de color distintos (el de la tarjeta y el del realce), en vez de uno solo. Se sacó ese realce: la cita va directo sobre el mismo fondo que el resto de la tarjeta, así elegir un color deja todo de ese único color — la cinta decorativa (dorada, fija) es el único acento que no cambia con el color elegido, igual que la comilla/regla de los otros estilos.
  - `card_style`/`card_color`, nuevas en `comments` (solo aplican con `kind = 'quote'`) y en `profile_quotes` — nullable, sin default a propósito: una cita ya publicada antes de esta migración no tiene ninguno de los dos, y sigue cayendo en el tratamiento genérico de siempre (`ActivityCard` solo usa el estilo nuevo si `card_style` no es `null`) — nada que migrar para atrás. Cuando SÍ hay una tarjeta-imagen guardada (`quote_image_url`, citas de club), esa sigue ganando siempre — es lo más fiel a lo que se eligió al publicar.
  - `CardStylePicker` (selector + vista previa en vivo, con el mismo `QuoteFeedCard` que va a mostrar el feed) se sumó en los tres lugares donde se escribe o edita una cita: `NewCommentForm`/`EditQuoteModal` (citas de club, como una sección aparte de "Estilo de la tarjeta") y `QuoteComposer` (citas del perfil). Elegir un estilo por primera vez arrastra el color por default de ESE estilo — pero solo mientras no se haya tocado el color a mano; en cuanto se toca una vez, queda fijo y cambiar de estilo ya no lo pisa.

**Se sacó el "Estilo de la tarjeta" para Instagram al crear o editar una cita (sin migración).** Con el estilo propio del feed ya andando (arriba), quedaban dos selectores de "estilo" en la misma pantalla con nombres parecidos — el viejo (`quote_style`/`quote_image_url`, migración 019: cover/dark/editorial, pensado para descargar como imagen y compartir en Instagram) y el nuevo (`card_style`/`card_color`, cómo se ve la cita DENTRO del feed de Libris). Se sacó el viejo por completo de `NewCommentForm`/`EditQuoteModal`: ya no hay forma de elegir ni regenerar una tarjeta-imagen al publicar o editar una cita — solo queda el selector de "Cómo se ve en el feed".

  - **Es solo un cambio en la creación/edición, no en los datos ni en la lectura**: `src/lib/quoteCard.js` (el motor que dibuja la tarjeta en un `<canvas>`), `QuoteCardPreview.jsx`, `DownloadQuoteImageButton.jsx` y el `CommentBody` de `ComentariosScreen.jsx` quedan intactos — una cita que ya tenía guardada su tarjeta de Instagram (de antes de este cambio) la sigue mostrando y se sigue pudiendo descargar exactamente igual que siempre. Simplemente ninguna cita nueva puede generar una desde ahora.
  - **Bug real encontrado al sacarlo, ya arreglado**: `updateQuote` (`clubs.js`) todavía mandaba `quote_style`/`quote_image_url` en su `update`, tomados del formulario — al sacar esos campos del formulario, las dos variables hubieran quedado siempre en `null`, así que **editar cualquier cita (aunque sea para corregir una palabra o cambiar el color del feed) hubiera borrado la tarjeta de Instagram ya guardada de esa cita**, junto con su archivo en el bucket `quote-cards`. Se corrigió sacando esos dos campos del `update` por completo — editar una cita ya no toca para nada su tarjeta vieja, la deja como estaba.
...
- **El orden de la tarjeta, al principio, era el de un posteo de Instagram** (primero la imagen sola y limpia, el texto después) — se invirtió más adelante al pedir una estética más de X; ver más abajo "Inicio y Perfil, estética más parecida a X".
...
En vez de una fila angosta por club, cada club en `/` es una tarjeta con el mecanismo visual del héroe: nombre del club arriba (fila propia, sin tocar), y debajo una fila con la portada a la izquierda (recortada a `object-fit: cover`, con la sombra apilada en diagonal) y a la derecha, en columna, el kicker "Leyendo ahora"/título/autor arriba, solos, y abajo del todo la barra de progreso — siempre continua, nunca pips — con el % pegado al final del relleno (no como número aparte) y la meta de progreso (página o capítulo) como pie de foto debajo de la barra. Todo alineado con el borde inferior de la portada — la barra ya no es una franja aparte que cruza toda la tarjeta, arranca donde termina la portada. Si un club todavía no tiene libro activo, su tarjeta muestra solo el nombre y "Todavía no tiene un libro activo".
...
- **Barra siempre continua, sin capítulos al lado del autor** — la barra de progreso "en pips" (un punto por capítulo, cuando el libro tenía pocos) se sacó: ahora es siempre la barra continua, sin importar si el progreso se registra por capítulo o por página. `computeHeroProgress` (`src/lib/heroProgress.js`) se simplificó a la par — ya no calcula `pips` ni `unit` (el "9 capítulos" que iba pegado al autor), solo `progressMeta` (la etiqueta chica de abajo del %). El autor, si el libro no tiene autor cargado, ahora no deja un punto (" · ") colgando — se probó igual con `unit`, así que se corrigió de paso.
- **% y página, integrados a la barra** — se probaron 4 mockups de dónde poner el % y la cantidad de páginas (apilados junto al título como estaba, en fila propia encima de la barra, % arriba con la página pegada a la barra, o todo junto a la barra) y se eligió esta última: título/autor quedan solos arriba, sin nada compitiendo al lado, y el % (`club.percent`, tamaño reducido a 14px para que entre en la fila) se muestra pegado al extremo derecho de la barra misma, con `club.progressMeta` como pie de foto chico debajo, alineado a la derecha.
- **Un degradado por posición, con textura de lino** — la tarjeta pasó de coral sólido a un degradado fuerte (claro arriba, tinta oscura abajo) con una trama de lino sutil encima (dos `repeating-linear-gradient` cruzados a 45°/-45°, `mix-blend-mode: overlay`). Se probaron varios pasos antes de llegar acá: primero un degradado sutil dentro del mismo coral, después degradados más marcados en distintos colores, y por último dos texturas (grano y lino) sobre esos colores — el lino fue el elegido. `CLUB_PALETTE` (`src/screens/MisClubesScreen.jsx`) tiene 4 combinaciones {degradado, color de anillo — para el borde de los avatares y del "+N"}: verde, violeta, azul y rosa. El color de cada tarjeta sale de su **posición** en la lista (`index % 4`), no de una elección de quien administra el club ni de un color fijo por club — mismo criterio que ya usa `Avatar.jsx` para elegir color por persona, sin pedirle nada a nadie. Como `getMyClubs` ordena por `joined_at` (nunca por actividad reciente), la posición — y por lo tanto el color — de cada club es estable en el tiempo, y el ciclo de 4 garantiza que dos tarjetas vecinas nunca compartan color (solo se repite entre clubes que ya no compiten por atención en la misma pantalla). La textura y el degradado van en una capa `position: absolute` propia dentro de la tarjeta (que ahora es `position: relative; overflow: hidden`); el resto del contenido (encabezado, portada, barra, integrantes) se marcó `position: relative` para pintarse por encima de esa capa.
...
### Fecha y lugar de la próxima reunión (migración 038)

Los administradores pueden cargar, en Preferencias, cuándo es la próxima reunión del club y dónde — con dos modalidades para el "dónde": un **link** (Zoom, Meet, lo que sea) o un **lugar físico** (dirección o nombre de local). Si hay fecha cargada, todo el club la ve arriba de todo en la pantalla del club (`/club/[clubId]`), con un botón que abre el link tal cual si es por videollamada, o un link a Google Maps armado con el texto del lugar si es presencial — **sin mapa embebido ni API key**: `googleMapsUrl()` (`src/lib/meetingFormat.js`) solo arma `https://www.google.com/maps/search/?api=1&query=<lugar>`, la búsqueda pública de Maps. Guardar coordenadas o pagar por un mapa interactivo hubiera sido pasarse de lo que hacía falta — un link que abre Maps con el lugar ya escrito resuelve "cómo llego" igual de bien.

- **4 columnas nuevas en `clubs`**: `meeting_at` (`timestamp` **sin** huso horario, a propósito — mismo criterio que ya usa el resto de la app para fechas, ver `formatRelativeTime.js`: lo que la persona administradora escribe en el `datetime-local` se guarda y se muestra tal cual, sin convertir entre husos), `meeting_mode` (`'link'` o `'lugar'`), `meeting_link`, `meeting_place`. Sin política de RLS nueva — la de "los administradores editan su club" (migración 009) ya cubre cualquier columna de la fila, y "members read their clubs" ya deja leer la fila entera a cualquier integrante.
- La reunión es **opcional y de todo o nada**: si se borra la fecha, `updateClubPreferences` (`src/app/actions/clubs.js`) pone las 4 columnas en `null` — no queda un link o un lugar "colgado" sin fecha.
- **`MeetingModeOption`** (componente local de `PreferenciasScreen.jsx`) — el mismo patrón visual de chip que el resto de la app, pero con un `<input type="radio">` nativo debajo (oculto), para que viaje en el `FormData` del formulario sin JS extra al enviar — igual que ya hace `VisibilityOption` para elegir la visibilidad del club.
- El bloque que ve el club (`ClubScreen.jsx`) es una tarjeta chica entre el encabezado y "Tu camino", solo si `club.meeting_at` existe — no compite con el resto de la pantalla si nadie cargó una reunión.
- **Autocompletado de direcciones** — mientras se escribe en "Dirección o nombre del lugar" aparecen sugerencias reales debajo del campo, vía la búsqueda pública de **Nominatim (OpenStreetMap)**, otra vez sin API key ni billing — mismo criterio que `googleMapsUrl()`. `searchPlaces` (Server Action, `src/app/actions/places.js`) hace el pedido del lado del servidor (Nominatim exige un User-Agent propio, que no tendría sentido mandar desde el navegador de cada persona) y solo responde a partir de 3 letras. **`PlaceAutocompleteInput`** (`src/components/PlaceAutocompleteInput.jsx`) es un input de texto normal — viaja en el `FormData` como cualquier otro campo, `name="meetingPlace"` — con el pedido debounceado 400ms para no perseguir cada tecla (Nominatim pide como máximo ~1 solicitud por segundo; con el debounce, el uso normal de la app queda lejos de ese límite). Elegir una sugerencia solo completa el texto del campo — no se guardan coordenadas, sigue siendo la misma dirección en texto libre que ya usa `googleMapsUrl()`. Lleva el crédito a OpenStreetMap debajo de la lista, como pide su política de uso.
- **Un aviso chico en "Mis clubes de lectura"** — se probaron 3 mockups de cuánto mostrar ahí (solo un ícono, una línea con fecha corta, una línea con fecha y lugar) y se eligió el primero: si `club.meeting_at` existe, una insignia circular se superpone a la esquina de la portada (calendario si `meeting_mode` es `'lugar'`, cámara si es `'link'`) — sin fecha ni lugar, para no sumarle más texto a una tarjeta que ya tiene bastante. El detalle completo sigue viéndose solo al entrar al club, en el bloque de `ClubScreen.jsx`.
- **La insignia es tocable** — se probó primero solo como ícono decorativo, pero la expectativa natural es que se pueda tocar. Es un `<button>` con el mismo `preventDefault`+`stopPropagation` que ya usa la fila de integrantes, así el toque no navega a la tarjeta entera.
- **Un paso previo antes de salir a Maps** — tocar la insignia abría Maps (o el link) directo, sin más contexto. Ahora abre primero **`MeetingInfoModal`** (`src/components/MeetingInfoModal.jsx`), la misma hoja (`Modal`) que ya usan otros paneles cortos de la app: muestra el horario (`formatMeetingDate()`) y, para un lugar físico, la dirección aparte, como su propia fila tocable — tocar **la dirección**, no la hoja entera, es lo que abre Maps (`googleMapsUrl()`). Para una reunión por link, la fila equivalente es el link mismo, que abre directo (no hace falta un paso extra ahí — no hay nada más que mostrar).
...
**Se sacó el switch "La portada ya trae el título"** de Preferencias (`book.cover_has_title`). Solo afectaba a la tarjeta de cita con estilo "Portada" (`renderCover` en `quoteCard.js`): si estaba activado (el default), no repetía el título encima de la imagen de fondo; desactivado, lo dibujaba. En la práctica era una utilidad muy angosta — el default ya acertaba en casi todos los casos (la portada real trae el título impreso) y el único momento en que el toggle importaba era con una portada recortada o "limpia" sin texto, un caso raro para justificar un control en la pantalla de Preferencias. Se sacó el checkbox, su estado y el campo del `FormData`/la acción del servidor; `quoteCard.js` quedó con el comportamiento que ya era el default (nunca vuelve a dibujar el título sobre la portada) en vez de leerlo de la base. La columna `cover_has_title` sigue existiendo en `books` — no se tocó ninguna migración, solo dejó de leerse y escribirse desde la app.

### Inicio y Perfil, estética más parecida a X

Se pidió acercar el look de `ActivityCard` (la tarjeta que arma el feed de Inicio y la lista de Perfil) a X/Twitter — **sin recrear su interfaz de marca** (sus íconos puntuales, las pestañas "Para ti"/"Siguiendo", el check de verificación): eso queda fuera. Lo que sí se trajo, dentro de la identidad de Libris (coral, crema, serif, sus propios íconos), fue la idea de fondo: un timeline más denso, con la fila de "me gusta"/"comentar" como íconos con número al lado en vez de píldoras de botón. Se probaron 3 mockups (timeline sin tarjetas, tarjeta compacta con borde fino, o solo cambiar la fila de acciones) y se eligió el primero.

- **Cada publicación es una fila, no una tarjeta** — `padding: '12px 0'` y un `borderBottom` fino (`var(--border-subtle)`) en vez de vivir dentro de una tarjeta con aire alrededor; `InicioScreen`/`PerfilScreen` sacaron el `gap` entre tarjetas de su lista (ya no hace falta, las separa la línea) y le sumaron un `borderTop` para que la primera publicación también tenga su línea arriba.
- **El texto va antes que la imagen** — al revés de como estaba (imagen primero, como un posteo de Instagram): ahora el bloque de texto (`metaText`, título/autor, la cita o el comentario, tocable para desplegarlo) se mueve arriba de la imagen. El botón de descargar la cita como imagen y el link "Ver el resto de los comentarios" pasan a vivir después de la fila de acciones, no adentro del bloque de texto — quedan como un cierre de la tarjeta expandida, no antes de la imagen.
- **El nombre lleva la hora al lado** ("Carla Guzmán · hace 2 h", en la misma línea) — antes la hora vivía aparte, en `metaText`, debajo de la imagen. En Inicio (donde sí hay `author`) la hora ahora sale del encabezado; `metaText` la repite solo cuando no hay encabezado con nombre (Perfil, donde por diseño no se repite de quién es cada tarjeta) — para no mostrarla dos veces en Inicio ni perderla en Perfil.
- **`compact` — la fila de acciones sin píldoras** — nuevo prop opcional en `LikeButton`, `EngagementBlock` y `PhotoCommentsBlock`: ícono + número, sin fondo ni relleno, en vez del botón con fondo `--accent-50`/`--surface-sunken` de siempre. Es opt-in y por default sigue siendo la píldora de toda la vida — `ActivityCard` es el único que pasa `compact`; `ComentariosScreen` (los comentarios del club) y las respuestas dentro de un hilo (`ReplyRow`, que ya usaba un `LikeButton` chico) no lo tocan, quedan exactamente como estaban. Sin este prop hubiera hecho falta bifurcar los tres componentes o duplicar su lógica de "me gusta"/responder — con un prop de presentación alcanza, el comportamiento (like optimista, abrir/cerrar el campo de responder, el hilo agrupado) es el mismo código de siempre.
- **Los divisores no llegan al borde real de la pantalla** — a diferencia del mockup (donde sí cruzaban de punta a punta), acá siguen inset por el `padding: '20px 18px 24px'` que ya traía cada pantalla: sacar ese padding solo para la lista de actividad hubiera significado reestructurar el resto de Perfil (info de perfil, formulario de editar, estadísticas), que no era parte de este pedido.

**Después, dos ajustes más sobre esta misma tarjeta:**

- **Los comentarios se despliegan en una hoja aparte, no adentro de la tarjeta** — primer intento: mostrar el hilo entero colapsado detrás de un solo "Ver N respuestas" ahí mismo, adentro de la tarjeta. No era lo que hacía falta (la tarjeta seguía creciendo con el largo de la conversación) — el patrón real es el de Instagram: como mucho 2 comentarios de vista previa (solo texto, sin acciones, con "Ver los N comentarios" debajo) y tocar el ícono de comentar (o esa vista previa) abre una hoja aparte (`Modal`) con el hilo completo y el campo para escribir, siempre visible ahí abajo — la tarjeta en sí nunca crece. Adentro de esa hoja, cada comentario raíz tiene su propio "Ver N respuestas" independiente (`ThreadGroup`, en `EngagementBlock`) — colapsado por default, no uno solo para toda la lista. Fuera de `compact` (`ComentariosScreen`, los comentarios del club) el hilo se sigue mostrando siempre en el lugar de siempre, sin hoja aparte — mismo criterio que ya regía para la píldora vs. el ícono+número.
- **El texto se corta a 5 líneas, no 2, con un "más" visible** — mismo tope que ya usaba `BookReviewCard` para la reseña (que no se tocó, ya cumplía esto). "más" solo aparece si el texto de verdad se corta: un `useEffect` compara `scrollHeight` contra `clientHeight` del bloque de texto después de pintar y solo entonces lo muestra — sin esa medición, cualquier cita cortita hubiera quedado con un "más" sin nada más para mostrar. Tocar el bloque entero lo sigue desplegando igual que antes; "más" es solo la pista visible de que hay más para leer, no reemplaza ese toque.
- **Se sacó `metaText`** — la línea chica ("Compartió una foto", "Publicó una cita", o el nombre del club) que iba arriba del título/cita/comentario. Sobraba: con el nombre y la hora ya en el encabezado (ver más arriba), esa línea repetía la misma idea de otra forma. Se fue entera, junto con los íconos de cámara/cita que la acompañaban — no quedó ningún reemplazo. **Efecto en Perfil**: como ahí no hay encabezado con nombre (no hace falta repetir de quién es cada tarjeta — ver "Inicio: feed de novedades" más arriba), esa línea era el único lugar donde se veía la hora de una foto, cita o comentario puntual; al sacarla, esas tarjetas quedan sin hora visible en Perfil (la reseña final no se toca — su encabezado con nombre + hora sigue igual, en las dos pantallas). Se dejó así, con este mismo criterio: si hace falta, se puede sumar de nuevo más adelante, en otro lugar de la tarjeta.
- **La foto de perfil pasó a su propia columna izquierda**, con nombre, texto, imagen y fila de acciones alineados a su derecha — igual que en X, comparando con una captura real. Antes el avatar y el nombre eran un único bloque en su propia fila (`display: flex` horizontal), y todo lo que iba debajo (el texto, la imagen, la fila de "me gusta") arrancaba desde el borde izquierdo de la tarjeta entera, sin alinearse con el nombre — quedaba una tarjeta con dos puntos de partida distintos en vez de una sola columna prolija. Ahora `avatarLink` (el avatar solo, como su propio link al perfil) va en una columna de ancho fijo a la izquierda, y `nameLink` (el nombre, sin el avatar) encabeza una columna a la derecha que contiene todo lo demás. Solo aplica cuando hay avatar (`author`) — al principio, eso era Inicio siempre, y en Perfil solo la reseña final.
- **La misma columna izquierda fija, en la reseña de Comentarios del club** — al pedir la misma dinámica "en el feed también", el único lugar que todavía quedaba con el patrón viejo (avatar+nombre en su fila, después el resto del contenido a ancho completo, sin alinear) era `ReviewCard` en `ComentariosScreen.jsx` — la reseña final que aparece arriba de todo al entrar a los comentarios de un club, con el mismo `BookReviewCard`+`EngagementBlock` que ya usa esta tarjeta en Inicio/Perfil (ver comentario en el código). El resto de `ComentariosScreen` (cada comentario de capítulo, cita o nota de voz, en el loop de `visibleComments`) ya tenía esta columna desde antes — no hizo falta tocarlo.
- **En Perfil, el nombre pasó a verse en las cinco (no solo en la reseña)** — capturas de la app en mano: en Perfil, `author` solo se pasaba para la reseña final ("ya se sabe de quién es el perfil, no hace falta repetirlo" — ver el comentario original en `ActivityCard.jsx`), así que las tarjetas de foto, cita, comentario y nota de voz no tenían nombre **ni avatar**, y por lo tanto tampoco columna izquierda para alinear el resto — quedaban con el patrón viejo por falta de "author", no por un error en el layout nuevo. `PerfilScreen.jsx` ahora pasa `author` siempre (con los datos del perfil que se está mirando), en las cinco — mismo criterio que ya regía en Inicio.
...
**Después, se pidió "más color"** — el fondo oscuro liso quedaba correcto pero un poco chato. Se armaron 3 mockups (un resplandor coral/dorado sobre el fondo oscuro con la textura de lino de las tarjetas de club, este mismo degradado a pleno color, y un aura de color detrás del logo) y se eligió el degradado a pleno color: `linear-gradient(160deg, var(--accent-500) 0%, var(--accent-600) 32%, var(--gold-700) 68%, #7A5A06 100%)`, coral arriba y dorado abajo, con la misma textura de lino que ya usan las tarjetas de club (`CLUB_TEXTURE` en `MisClubesScreen.jsx` — se repite acá en vez de importarse, es un detalle interno de ese archivo). El botón "Empezar" dejó de usar el `<Button>` del sistema de diseño — su `variant="primary"` es fondo coral, que sobre este degradado desaparece, y `<Button>` no deja pasar un color propio (su `style` interno pisa cualquier `style` que se le pase por props, mismo problema ya conocido de otras pantallas); quedó un link con la misma métrica (radio, padding, tipografía) pero fondo oscuro (`--neutral-900`), a mano.

**Y después, "Empezar" más arriba** — con `justifyContent: 'space-between'` entre 3 bloques (espacio vacío, contenido, botón), el botón quedaba pegado abajo de todo, lejos del texto que lo justifica. El botón se unió al mismo bloque que el logo/texto (un solo grupo, con `gap`) y ese grupo entero sube con un `paddingTop: '18vh'` en vez de repartirse por el alto completo de la pantalla — el aire vacío que sobra queda abajo, no partido entre arriba y abajo.

### Repostear el feed de otras personas, y GIF en las fotos

Dos preguntas seguidas — "¿se pueden compartir publicaciones de otros en el feed?" y "¿se pueden subir GIF?" — resultaron ser dos cosas que no existían todavía (lo que sí existía, `ShareButton`/`toggleShareToFeed`, solo deja compartir TU PROPIO comentario/nota de voz a Inicio; no había forma de reenviar algo de otra persona). Migración 039.

**Repostear** — `RepostButton` (`src/components/RepostButton.jsx`), ícono+número igual que `LikeButton`, en la fila de acciones de `ActivityCard` (reseña, cita, foto, y el comentario/nota de voz que ya se compartió a Inicio) — no vive en Comentarios del club, solo en Inicio/Perfil. Tocarlo llama a `toggleRepost(kind, id)` (`src/app/actions/reposts.js`), que inserta o borra una fila en la tabla nueva **`reposts`** (`profile_id` + `comment_id` **o** `post_id`, nunca los dos — una tabla para los dos tipos, a diferencia de los likes que tienen `comment_likes`/`post_likes` separadas, porque acá el mismo botón sobre la misma tarjeta cubre cualquier tipo de contenido).

`recent_activity` y `profile_activity` ganaron dos ramas más cada una (antes tenían "comentarios" y "fotos"; ahora también "comentarios reposteados" y "fotos reposteadas"), con `created_at` del repost —no del original— para que aparezca arriba, como algo fresco. El `id` de la fila sigue siendo siempre el del contenido ORIGINAL, reposteado o no: así "me gusta" de un repost sigue apuntando al mismo lugar que la publicación orgánica (ver más abajo, migración 040, por qué **comentar** es distinto). Como el mismo contenido puede aparecer dos veces (una vez publicado por su autor, otra reposteado por alguien que te sigue) —a propósito, es lo mismo que hace X— el `id` puede repetirse entre dos tarjetas distintas; el `key` de React en `InicioScreen`/`PerfilScreen` usa `repost_id ?? id`, no `id` a secas.

`ActivityCard` muestra, arriba de todo, "Fulana compartió esto" (o "Tú compartiste esto") cuando la tarjeta es un repost — pero el resto (avatar, nombre, contenido) sigue mostrando siempre al autor ORIGINAL, nunca a quien reposteó. Esto obligó a un cambio en **`PerfilScreen`**: antes `author`/`isOwn` se armaban con los datos de `profile` (la página que se está mirando) para cada tarjeta, porque siempre era la misma persona — con reposts eso ya no es cierto (podés ver, en el perfil de alguien, algo que esa persona reposteó de un tercero), así que ahora se arman con los datos de CADA item (`item.profile_id/display_name/avatar_url`, que las funciones ya resuelven al autor correcto) — para el resto de las tarjetas (no reposteadas) da exactamente lo mismo.

Qué se puede repostear lo valida la política de RLS de `insert` en `reposts`, con una función security definer nueva (`can_repost_comment`, mismo patrón que `is_club_member`): el SELECT normal de `comments` exige ser miembro del club, más angosto que la visibilidad real del feed (que también deja ver clubes públicos sin ser miembro) — sin esa función aparte, repostear una cita de un club abierto ajeno se hubiera rechazado por error, aunque esa cita sí se puede ver en Inicio.

**GIF** — el input de subir foto (`PostComposer`) ahora acepta `image/gif` además de jpeg/png/webp. Es la única excepción al recorte de siempre (`PhotoCropModal`): recortar ahí pasa por un `<canvas>`, que solo puede capturar un frame — recortar un GIF lo dejaría animado por dentro pero estático al mostrarlo. Un GIF salta directo a la vista previa con el archivo tal cual se eligió, y se sube sin tocar. El bucket `post-photos` (Storage) también tenía su propia lista de `allowed_mime_types` — hacía falta sumarle `image/gif` ahí también, o la subida se hubiera rechazado igual aunque el código de la app ya lo permitiera.

**Después, se pidió separar los comentarios de un repost de los del post original** — "cuando compartí algo y comenté en la publicación que compartí, ese comentario no debería aparecer en la publicación del usuario original, solo en la mía". Hasta acá, comentar desde un repost creaba una respuesta normal (`parent_comment_id` al comentario/cita/reseña original), así que aparecía en TODOS lados donde se ve el original — incluida la pantalla de Comentarios del club de quien lo escribió, que nunca reposteó nada. Migración 040.

- **`repost_id`** — columna nueva, nullable, en `comments` y en `post_comments` (`on delete cascade`: dejar de repostear se lleva puesto lo que se comentó ahí, no tiene sentido dejarlo huérfano). `null` es un comentario normal, como siempre; con valor, el comentario vive scopeado a ESE repost puntual — no al contenido en general. Cada repost tiene ahora su propio hilo, separado tanto del original como de cualquier OTRO repost del mismo contenido (si dos personas repostean la misma cita, cada una tiene su propia conversación aparte, no comparten una).
- **`ActivityCard`** calcula `repostId` (`activity.repost_id` cuando la tarjeta es un repost) y se lo pasa a `EngagementBlock`/`PhotoCommentsBlock`, que lo mandan en el `FormData` de `postReply`/`postPhotoComment` si está presente. `ComentariosScreen` nunca lo pasa — ahí no hay reposts, todo sigue siendo `repost_id is null` como antes de esta migración.
- **`recent_activity`/`profile_activity`**: las ramas "orgánicas" (comentarios y fotos publicados por su autor) filtran `repost_id is null` en su subconsulta de respuestas; las ramas "reposteadas" filtran `repost_id = <el id de ESE repost>` — así cada tarjeta trae solo su propio hilo, nunca el ajeno.
- **`getChapterCommentCounts`** (la pastilla de "Comentarios" en Tu camino) y el `select` de `comments` en la pantalla de Comentarios del club ahora también filtran `repost_id is null` — sin esto, un comentario hecho en el repost de una cita de ese club se sumaba igual al conteo del club, o aparecía colgando del original en esa pantalla — justo lo que no debía pasar.
- **RLS**: la política de insert de `comments` ya exigía ser miembro del club (`is_club_member`) — correcto para un comentario normal, pero un repost puede verse en Inicio sin ser miembro (club público), así que se sumó una rama nueva con `can_comment_on_repost` (mismo patrón que `can_repost_comment` de la 039): cualquiera que pueda VER el repost puede comentarlo, sea o no quien lo compartió. Mismo criterio para `post_comments`. `postReply` también dejó de leer `club_book_id`/`chapter_id` del original con un `select` directo (sujeto a ese mismo RLS más angosto) y pasó a `get_comment_context`, una función aparte security definer — ese dato es solo estructural (a qué libro/capítulo pertenece), la autorización de verdad sigue viviendo en la política de insert.
- **`notifications_feed`** — un comentario scopeado a un repost ya no genera la notificación "te respondieron" para quien escribió el original (no es una respuesta a SU publicación). Queda pendiente, a propósito, avisarle a quien reposteó que le comentaron "lo que compartió" — no se pidió, y es un tipo de notificación nuevo de verdad, no un ajuste de este cambio; por ahora ese comentario no le avisa a nadie.

### Foto o GIF al comentar un capítulo

Se preguntó si se puede adjuntar una foto o GIF al comentar un capítulo — no se podía: `NewCommentForm` (Comentarios del club) era puro texto/cita, sin ningún input de archivo. Migración 041, solo para **Comentario** (`kind = 'text'`) — una cita ya tiene su propia imagen (la tarjeta armada), no hacía falta sumarle otra cosa ahí.

- **`comments.image_url`** — columna nueva, guarda el **path** del archivo (no una URL pública). A diferencia de "quote-cards" o "post-photos" (públicos, pensados para compartir afuera), la foto de un comentario va a un bucket **privado**, `comment-photos` — es una conversación de adentro de un club, mismo criterio que ya usan las notas de voz (`voice-notes`). El path es `<club_book_id>/<profile_id>/<archivo>`: el primer segmento habilita el chequeo de membresía (leer/subir, RLS de Storage calcado de `voice-notes`), el segundo el de dueño (borrar, calcado de `post-photos`/`quote-cards`).
- **`src/lib/commentPhotos.js`** (`signCommentImageUrls`) — como el path no es una URL, hay que firmarla del lado del servidor antes de mandarla al navegador. Se llama en los tres lugares donde un comentario con foto puede aparecer: la pantalla de Comentarios del club, Inicio y Perfil (si ese comentario se compartió al feed con una foto puesta) — mismo patrón que ya usaba esa página para firmar el audio de las notas de voz, generalizado a cualquier fila con un campo `image_url`.
- **`NewCommentForm`** — un botón "Agregar foto o GIF" (solo visible con **Comentario** elegido, no con **Cita destacada**) abre el selector de archivos; la vista previa es un thumbnail chico con una "x" para sacarla. Un JPG/PNG/WEBP pasa por `compressImage` (achica sin recortar, ya usado para la portada del libro); un GIF NO — mismo motivo de siempre, un `<canvas>` solo captura un frame y lo dejaría estático.
- **`postComment`** sube el archivo (si llega, "mejor esfuerzo": si falla, el comentario se publica igual, sin foto) a `comment-photos` y guarda el path en `image_url`. `deleteComment` ahora también borra ese archivo de Storage al borrar el comentario (mismo patrón que `deletePost` con las fotos de Perfil) — editar el comentario (`updateComment`) sigue siendo solo texto, la foto no se reemplaza, mismo criterio que ya rige para fotos y citas en el resto de la app.
- **`ComentariosScreen`** (`CommentBody`) y **`ActivityCard`** muestran la foto cuando existe: en Comentarios, debajo del texto; en el feed (Inicio/Perfil, si el comentario se compartió), ocupa el lugar que antes tenía la portada del libro de fondo — la portada seguía siendo el "algo visual" por default para un comentario de texto, ahora es la foto real cuando la hay.
- **`recent_activity`/`profile_activity`** ganaron una columna `image_url` más (junto a `photo_url`) en sus cuatro ramas — `null` salvo en un comentario de texto que la tenga puesta.

**Después, se pidió carrusel (varias fotos, no una sola)** — migración 042, todavía sobre lo mismo (Comentario de capítulo, mostrado en Comentarios del club, Inicio y Perfil).

- **`comment_photos`** reemplaza a `comments.image_url`: una tabla aparte (`comment_id`, `path`, `position`) en vez de una columna de texto — hacía falta guardar el ORDEN del carrusel, algo que una sola columna no puede. La migración 042 migra la foto suelta de la 041 a esta tabla (posición 0) y borra la columna vieja — escrita para andar tanto si la 041 ya se había corrido como si no.
- **De paso, una corrección real de visibilidad**: la política de Storage que agregó la 041 para LEER las fotos exigía ser miembro del club (`is_club_member`) a secas — pero `recent_activity` ya deja ver un comentario compartido de un club **público** sin ser miembro. Con esa política tal cual, la foto de ese comentario nunca se hubiera podido firmar para alguien así, aunque el comentario en sí sí se viera en Inicio — mismo tipo de bug de "RLS más angosto que la visibilidad real" que ya aparecieron con los reposts (migraciones 039/040). Se reemplazó por la misma regla que ya usan recent_activity/profile_activity: club sin invitación, o sos miembro.
- **`src/lib/commentPhotos.js`** (`signCommentImageUrls`) pasó de firmar un path suelto (`image_url`) a firmar un arreglo (`image_urls`) — mismo mecanismo, ahora sobre varios paths por fila.
- **`src/components/PhotoCarousel.jsx`** — nuevo, sin ninguna librería (scroll-snap nativo + puntitos abajo, que solo aparecen si hay más de una foto). Se usa tanto en `ComentariosScreen` (`CommentBody`) como en `ActivityCard` — ahí reemplaza al `<div>` de fondo de siempre solo cuando el comentario tiene fotos; sin fotos, sigue siendo la portada del libro como fondo, sin cambios.
- **`NewCommentForm`** — el selector de archivo pasa a `multiple`, hasta 6 fotos/GIF por comentario (`MAX_COMMENT_IMAGES`, espejada en `postComment`). Cada una se ve como su propio thumbnail chico con su "x" para sacarla, y "Agregar más" mientras no se llegue al tope.
- **`postComment`** ahora inserta el comentario primero (necesita su `id` para las filas de `comment_photos`) y recién después sube cada foto — "mejor esfuerzo" por archivo: una que falle no tira abajo a las demás. `deleteComment` borra todos los archivos de Storage de ese comentario (las filas de `comment_photos` se van solas, `on delete cascade`, pero eso no borra el archivo real — hay que pedirlo aparte, y antes de borrar la fila del comentario).

**Reporte: "subí un carrusel y no apareció"** — migración 043, arregla un bug de la 042.

El comentario se publicaba bien y la foto se subía y se guardaba en `comment_photos` sin ningún error — el problema estaba al LEER: `createSignedUrls` rechazaba absolutamente todas las fotos, siempre, con el mismo mensaje genérico de Supabase ("Either the object does not exist or you do not have access to it") que no distingue "no existe" de "RLS lo bloquea". La causa: la política de SELECT de Storage que la 042 armó para ampliar la visibilidad hacía `join public.clubs cl on cl.id = cb.club_id` y después llamaba `storage.foldername(name)` — pero `clubs` también tiene una columna `name` (el nombre del club), y Postgres, sin ningún error ni aviso de ambigüedad, resolvía ese `name` suelto hacia la tabla más cercana en el `FROM` del subquery (`cl.name`) en vez de hacia el archivo real (`storage.objects.name`, la fila que la política evalúa). Terminaba comparando el `club_book_id` contra un pedazo del **nombre del club**, algo que nunca iba a coincidir — la política daba falso siempre, para cualquier foto, de cualquier comentario. La política de INSERT no tenía este bug (no hace `join clubs`, no hay ninguna otra columna `name` de por medio), por eso subir funcionaba perfecto mientras leer fallaba siempre: dos políticas del mismo bucket, evaluando cosas distintas.

Se encontró revisando los logs de Vercel en vivo (`get_runtime_errors`/`get_runtime_logs`, MCP de Vercel) con unos `console.log`/`console.error` temporales agregados en `postComment`, `signCommentImageUrls` y el select de `comentarios/page.js` — confirmaron que la escritura no fallaba nunca pero la firma sí, siempre, para cualquier foto de cualquier comentario — y confirmando la política exacta instalada con `select policyname, cmd, roles, qual from pg_policies where schemaname='storage' and tablename='objects'`, que mostró el `qual` real con `(storage.foldername(cl.name))[1]` en vez de `(storage.foldername(storage.objects.name))[1]`. Esos logs temporales ya se sacaron del código; el fix (calificar `storage.objects.name` explícitamente) queda en la migración 043.
