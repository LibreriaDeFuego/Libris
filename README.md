...
- **"Mostrar quién está leyendo"** — un botón chico debajo de "Tocá un capítulo..." (apagado por defecto) que agrega, junto a cada capítulo, una pastilla con quiénes de tus compañeros van por ahí: puntitos de color apilados (hasta 3, con "+N" si sobran — mismo criterio de color que `Avatar.jsx`, así el puntito y el avatar de arriba son la misma persona) y "Fulano" o "Fulano y N más" (mismo patrón de texto que ya usa "Actividad del club" para agrupar comentarios). Usa los mismos datos que ya trae `MemberProgressStrip` (`members`, prop nueva de `ChapterPath`) — nada de consultas nuevas. El botón ni aparece si nadie más tiene progreso registrado en ese libro. Se probaron 4 lugares para esto (al lado del nodo, franja de arriba conectada con un anillo, esta pastilla mini, y un carril lateral aparte) y se probó la pastilla con hasta 20 personas en un mismo capítulo antes de elegirla — el tamaño no crece más allá de los 3 puntitos + el "+N".
- **Cuántos comentarios tiene cada capítulo** se ve siempre, sin botón — es información útil de entrada, a diferencia de "Quiénes está leyendo" (`getChapterCommentCounts`, `src/lib/clubDetail.js`: cuenta `chapter_id` de la tabla `comments`, sin traer texto). No hay pastilla si el capítulo no tiene ninguno.
  - Si el capítulo está **al día o atrás** de tu progreso, la pastilla es un link directo a `/club/[clubId]/comentarios?capitulo=<id>` — la pantalla de Comentarios ganó soporte para `?capitulo=` (prop `initialChapterId` en `ComentariosScreen`, cae al primer capítulo si el id no es válido) para abrir ya parada en el capítulo que corresponde. Entrando así (por un capítulo puntual, no por el ícono de "Comentarios del club"), la pantalla se queda corta a propósito: **no muestra las reseñas finales** (son del libro entero, casi siempre con spoilers del final) ni el selector para cambiar de capítulo (solo el nombre, como texto — nada de "Ver otro capítulo": entraste a ver ESE capítulo) — y el **orden se invierte**: primero lo que ya se comentó, el cuadro para dejar tu propio comentario/cita/nota de voz queda al final, después de leer lo que ya se dijo. Entrando por el ícono de "Comentarios del club" se sigue viendo todo como antes: selector completo arriba, cuadro para comentar antes que la lista.
...
- **Terminar el libro desde el propio camino** — al llegar al último capítulo (tocado como propio), arriba de su nodo aparece uno más: libro cerrado + "FIN" chico debajo, con borde punteado dorado, en el lugar donde antes iba "+N capítulos más adelante" (que ya no aplica, no queda nada más adelante). Tocarlo llama a la misma Server Action `updateProgress` con `mode: 'finished'` que ya usaba el modal completo, y dispara el mismo flujo de siempre: se cierra, y se abre `FinalReviewModal` para dejar la reseña. Se probaron varias formas (banner aparte, nodo con trofeo, 5 íconos alternativos) antes de llegar a esta — un nodo más del camino, no un cartel aparte, con libro cerrado + "FIN" juntos en vez de elegir entre uno u otro.
