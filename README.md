...
- **Las respuestas ya puestas arrancan colapsadas** — antes `EngagementBlock` y `PhotoCommentsBlock` mostraban siempre el hilo entero apenas había algo; ahora, con `compact`, arrancan escondidas detrás de un "Ver N respuestas"/"Ver N comentarios" (texto plano, sin píldora, mismo tono que el resto de la fila de acciones) — tocarlo las despliega. Si se abre el campo para escribir una respuesta nueva (tocando el ícono de comentar, o "Responder" en una respuesta puesta), las existentes se despliegan solas, para no escribir sin ver la conversación. Fuera de `compact` (`ComentariosScreen`, los comentarios del club) el hilo se sigue mostrando siempre, sin este colapso — mismo criterio que ya regía para la píldora vs. el ícono+número.
