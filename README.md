...
- **Un carril de color marca el volumen de cada capítulo**, cuando el libro tiene más de uno (`ChapterPath` recibe `volumes` además de `chapters`). Un color fijo por volumen — no tiene nada que ver con `pathColor`, que sigue marcando lo leído — corrido al lado del camino, con el nombre real del volumen (el mismo que le pusiste en "Gestionar capítulos") la primera vez que aparece cada uno. Se probaron otras formas (un encabezado de texto entre volúmenes, una barra de resumen arriba del camino, un nodo punteado tipo "inicio de volumen") y esta — el carril corrido — fue la elegida, porque se nota el cambio de volumen sin tener que llegar al final de uno. Con un solo volumen (o ninguno, el caso de siempre hasta ahora) no se muestra nada — el camino queda exactamente igual que antes.
