...
- **Terminar el libro desde el propio camino** — al llegar al último capítulo (tocado como propio), debajo de su nodo aparece uno más, al final del camino: un libro cerrado con borde punteado dorado, y "FIN" como título abajo (se probó el ícono y el texto juntos adentro del mismo círculo chico, pero se veía apretado — separarlos, el libro solo adentro y "FIN" como título debajo, se lee mejor). Tocarlo llama a la misma Server Action `updateProgress` con `mode: 'finished'` que ya usaba el modal completo, y dispara el mismo flujo de siempre: se cierra, y se abre `FinalReviewModal` para dejar la reseña. Se probaron varias formas antes de llegar a esta (banner aparte, nodo con trofeo, 5 íconos alternativos) — un nodo más del camino, no un cartel aparte.
