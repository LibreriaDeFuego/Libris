...
En vez de una fila angosta por club, cada club en `/` es una tarjeta de color sólido con el mecanismo visual del héroe: la portada ocupa toda la franja izquierda de la tarjeta, de punta a punta (alto completo, borde con borde, con un degradado sutil donde se une al coral) — no una miniatura chica al costado. A la derecha, en columna: nombre del club arriba, kicker "Leyendo ahora"/título/autor/capítulos, % y capítulo actual, y la barra de progreso (pips o continua, según `computeHeroProgress`) ocupando el ancho completo de esa columna, sin el chrome que tenía el héroe grande (no hace falta selector de club ni íconos de acciones acá, cada tarjeta ya es un club distinto). Si un club todavía no tiene libro activo, no hay portada que mostrar — la tarjeta vuelve a ser una sola columna, con el nombre y "Todavía no tiene un libro activo".
...
- **La portada a todo lo alto** fue un ajuste posterior a la primera versión de esta tarjeta (que la mostraba chica, con sombra apilada en diagonal) — se pidió que pesara más. `ClubHeroCard` pasó de un `<div>` con padding uniforme a un contenedor flex (`align-items: stretch`) con la imagen como primer hijo de ancho fijo (`COVER_BAND_WIDTH`) y el resto del contenido como columna a la derecha, así la imagen se estira sola hasta igualar el alto de esa columna sin necesitar saber su tamaño de antemano.
