...
- **La insignia es tocable** — se probó primero solo como ícono decorativo, pero la expectativa natural es que se pueda tocar. Ahora es un `<button>` con el mismo `preventDefault`+`stopPropagation` que ya usa la fila de integrantes, así el toque no navega a la tarjeta entera: abre directo el link de la reunión, o Maps con el lugar (`googleMapsUrl()`), en una pestaña nueva.
