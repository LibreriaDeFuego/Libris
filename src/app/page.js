...
export default async function Page() {
  ...
  const [{ data: club }, { count: memberCount }, { data: clubBook }] = await Promise.all([
    supabase.from('clubs').select('id, name, is_private').eq('id', clubId).single(),
  ...
  const [{ data: chapters }, { data: myProgress }, { data: comments }, { count: otherClubsCount }] = await Promise.all([
  ...
  return (
    <ClubScreen
      ...
      previews={comments ?? []}
      otherClubsCount={otherClubsCount ?? 0}
    />
  );
}
