...
// escribir una nueva si se tocó "Comentar". El botón principal dice
// "Comentar", no "Responder" — es la acción de dejar un comentario, aunque
// por dentro quede guardado como una respuesta (parent_comment_id) al
// original. Todo en un solo componente porque el botón que abre el campo y
// el campo en sí comparten el mismo estado (si no, habría que subirlo a
// quien use esto).
//
// Cada respuesta ya puesta tiene, además, su propio "Responder" — como en
// Instagram: no crea un segundo nivel de hilo (todo sigue colgando del
// mismo original, un solo nivel de anidamiento, a propósito), solo
// precarga el campo compartido con "@Nombre " para que quede claro a
// quién le está contestando. El "@Nombre" queda como texto plano en el
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <button
          type="button"
          onClick={(e) => { e.stopPropagation(); if (replyOpen) closeReply(); else openReply(null); }}
          style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {replyingToName && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>
                  ...
                  <button
                    type="button"
                    onClick={() => setReplyingToName(null)}
